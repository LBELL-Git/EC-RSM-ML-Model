...
def loadTrainingData(path, fileName):
    ...
    file = pd.ExcelFile(r'./Data/ProcessedDataRRAM.xlsx')
...
#Load the data
trainingInputFeatures, trainingTargetLabels, datacopy = loadTrainingData('./Data','/ProcesssedDataRRAm.xlsx')
...
def loadExperimentData(path, fileName):
    ...
    """
    # value is hard coded, has to implement the fileName and path style declaration
    file = pd.ExcelFile(r'./Data/AI-Design-ML-CC-SiO-Experiment.xlsx')
    data = pd.read_excel(file, "SiO-Experiment")
    #check if the dataFrame is empty
    if data.empty:
        print("Load Data Error: Data not found in path")
        return
...
#Load the data
experimentInputFeatures, experimentTargetLabels, datacopy = loadExperimentData('./Data','/AI-Design-ML-CC-SiO-Experiment.xlsx')
...
def writeMLPModelPredictionsToExcel(test_labels, predicted_labels):
    ...
    df.to_excel('./Data/AI-Design-ML-CC-Experiment-SiO-MLPModel.xlsx', sheet_name='MLPModel', index=False)
    return
...
def writeRFModelPredictionsToExcel(test_labels, predicted_labels):
    df = pd.DataFrame({'Target':test_labels,'Predicted': predicted_labels})
    df.to_excel('./Data/AI-Design-ML-CC-Experiment-SiO-RFModel.xlsx', sheet_name='RFModel', index=False)
    return
